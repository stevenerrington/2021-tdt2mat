...
data.info.starttime = datestr(datenum([1970, 1, 1, 0, 0, headerStruct.start_time]),'HH:MM:SS:FFF');
data.info.stoptime = datestr(datenum([1970, 1, 1, 0, 0, headerStruct.stop_time]),'HH:MM:SS:FFF');
...
if ~useOutsideHeaders
    ...
    if mod(numel(heads),10) ~= 0, heads = heads(1:(10*floor(numel(heads)/10))); end
...
%     if ~strcmp(STORE, '') && ~ismember(currentName, STORE), continue; end
    if ~isempty(STORE) && ~any(ismember(currentName, STORE)), continue; end
    currentType = currentTypes{i};
    ind = find(strcmpi(ALLOWED_TYPES, currentType));
%     if ~any(TYPE==ind), continue; end
    if ~any(ismember(TYPE,ind)), continue; end
