...
for sessionLoopIdx = 1:length(uniqueSessionList)
    ...
    for penIdx = 1:nPenetrations
        ...
        spk_flag = str2num(ephysLog.spk_session{penLogIdx(penIdx)});
        lfp_flag = str2num(ephysLog.lfp_session{penLogIdx(penIdx)});
        ...
        ops.rootZ = fullfile('S:\Users\Current Lab Members\Steven Errington\2021_DaJo\spk\',ephysLog.Session{penLogIdx(penIdx)});
        
        try
            [spkTable] = phy2mat_infoOut(ops);
            spkInfo.unitInfo = spkTable;
            
            spkLog = importOnlineSpkCuration();
            spkLog_idx_session = [];
            spkLog_idx_session = find(strcmp(spkLog.Session,ephysLog.Session{penLogIdx(penIdx)}) == 1);
            
            spkInfo.unitInfo.flag_mua = cellfun(@str2num,spkLog.muaFlag(spkLog_idx_session));
            spkInfo.unitInfo.flag_noise = cellfun(@str2num,spkLog.noiseFlag(spkLog_idx_session));     
        catch
            spkInfo.unitInfo = [];
        end
        ...
        penTable(penIdx,:) = ...
            table({dataFilename}, {area}, spk_flag, lfp_flag, gridLoc,...
            recDepth, {recAngle}, {electrode}, spacing,...
            {lfpDatafile}, {spkDatafile}, lfpInfo, spkInfo,...
            'VariableNames', {'dataFilename','area','spk_flag','lfp_flag',...
            'grid','depth','angle','electrode','spacing',...
